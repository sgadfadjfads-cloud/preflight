...
> **核心规则：** 每个新任务开始时，先激活“问题本质闸门”，再决定如何行动。先确认受影响的人、真实需求、约束、受保护边界、成功条件和关键未知项。
...
把下面这句话放进你自己的 `AGENTS.md` 或同等的 Agent 指令文件中：

> 判断要深，表达要简洁；发现风险要说明理由，但不能擅自替用户做价值判断或越过授权边界。

这句话是 Preflight 最小但最有价值的版本。完整的 [`AGENTS.md`](AGENTS.md) 模板还补充了任务本质、成功条件、多轮校准和验证指导。
